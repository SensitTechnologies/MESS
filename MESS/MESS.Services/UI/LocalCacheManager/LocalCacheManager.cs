...
public class LocalCacheManager : ILocalCacheManager
{
    ...
    /// <inheritdoc />
    public async Task<List<ProductionLogCacheDTO>> GetProductionLogBatchAsync()
    {
        try
        {
            var result = await _protectedLocalStorage.GetAsync<List<ProductionLogCacheDTO>>(PRODUCTION_LOG_BATCH_KEY);
    ...
    public async Task<ProductionLogCacheDTO> GetProductionLogFormAsync()
    {
        try
        {
            var result = await _protectedLocalStorage.GetAsync<ProductionLogCacheDTO>(PRODUCTION_LOG_FORM_KEY);
